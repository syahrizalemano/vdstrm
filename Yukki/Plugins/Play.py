...
async def play(_, message: Message):
    ...
    if audio:
        mystic = await message.reply_text(
            "🔄 Process Audio... Tunggu Sebentar KNtl!"
        )
        try:
            read = db_mem[message.chat.id]["live_check"]
            if read:
                return await mystic.edit(
                    "Live Streaming Playing...Hentikan pemutar musiknya dulu asu"
                )
            else:
                pass
        except:
        ...
        mystic = await message.reply_text(
            "🔄 Processing Video... Tunggu Sbntr KNtll!"
        )
        try:
            read = db_mem[message.chat.id]["live_check"]
            if read:
                return await mystic.edit(
                    "Live Streaming Playing...Hentikan Pemutar Musiknya dlu asu"
                )
            else:
                pass
        except:
    ...
    try:
        ...
        if read1:
            return await CallbackQuery.answer(
                "Live Streaming Playing...Hentikan PEmutar Musik",
                show_alert=True,
            )
        else:
            pass
